...
echo "apt update -y | awk '{ print strftime("[%Y-%m-%d %H:%M:%S]"), $0 }' > test1"$(date +%B)".txt" >> /root/script/update.sh
echo "apt-get upgrade -o Dir::Etc::SourceList=/etc/apt/security.sources.list -y | awk '{ print strftime("[%Y-%m-%d %H:%M:%S]"), $0 }' > test1"$(date +%B)".txt" >> /root/script/update.sh
